...
def main():
    ...
    print(f"--- Đang quét {channel_name} (Sử dụng Flat-Playlist) để lấy {requested_count} video ---")
    
    # Sử dụng flat-playlist để lấy thông tin cực nhanh mà không bị YouTube chặn
    cmd_list = [
        "yt-dlp", 
        "--flat-playlist", 
        "-j",
    ...
    try:
        ...
        for line in res.stdout.strip().split('\n'):
            ...
            # Lấy thông tin từ flat playlist (nhanh, không cần fetch từng video)
            new_videos.append({
                'Video_URL': v_url,
                'Title': v.get('title', 'N/A'),
                'Description': 'N/A (Flat-Playlist)', # Flat playlist không có description
                'Thumbnail': v.get('thumbnails', [{}])[0].get('url', 'N/A'),
                'Channel_URL': channel_url
            })
            print(f"✅ Đã bốc: {v.get('title')[:40]}...")
    ...
    print(f"📊 Requested: {requested_count} videos")
    print(f"🆕 New videos collected: {len(new_videos)}")
    print(f"\n📁 Saved to Excel:")
